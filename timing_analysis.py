...
###################################################################################################################
    ...
    data = pd.read_csv('C:\\Users\\billv\\3project\\pythoncode\\data2.csv', header = None)
    df = pd.DataFrame(data).astype(float)
    df.columns = ['Rise Time', 'Settling Time', 'Overshoot']
    df.index  = num_list
    rt = df.iloc[:, 0]*pow(10,9)
    st = df.iloc[:, 1]*pow(10,9)
    ov = df.iloc[:, 2]
    ...
    '''
    plt.figure()
    plt.scatter(x = df.iloc[:, 1]*pow(10,9), y =  df.iloc[:, 2], s = 75, c =df.iloc[:, 0]*pow(10,9), cmap = 'Oranges')
    
    for i, n in enumerate(num_list):
        plt.annotate(n, (df.iloc[i, 1]*pow(10,9), df.iloc[i, 2]), xytext = (df.iloc[i, 1]*pow(10,9) + 0.005, df.iloc[i, 2] + 0.005))
    

   
    plt.title('Sampling Rate Performance Characteristics')
    plt.xlabel('Settling Time')
    ...
    norm = matplotlib.colors.Normalize(min(rt), max(rt))

    cb1 = matplotlib.colorbar.ColorbarBase(ax, cmap=cmap,
                                norm=norm,
                                orientation='horizontal')
    cb1.set_label('Rise Time (ns)')
    plt.show()
